...
public:
    ...
    {
        SetScheme("ui/resources/ClientScheme.res");
    }
...
void VControlsListPanel::StartCaptureMode( HCursor hCursor )
{
    ...
    if (hCursor)
    {
        ...
        if ( hCursor == dc_none )
        {
            ...
            // gEngfuncs->SetCursorPos(  engine->GetWindowCenterX(), engine->GetWindowCenterY() ); // use gEngfuncs and NOT input() because input() is relative to vgui, not absolute
        }
    }
