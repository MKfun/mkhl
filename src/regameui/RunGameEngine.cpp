...
public:
	...
	{
		gEngfuncs.pfnClientCmd((char *)text);
		return true;
	}
	...
	{
		return gEngfuncs.pfnGetLevelName() && strlen(gEngfuncs.pfnGetLevelName()) > 0;
	}
	...
	virtual void SetTrackerUserID(int trackerID)
	{
		...
		gEngfuncs.PlayerInfo_SetValueForKey("*tracker", buf);
	}
	...
	{
		int userID = gEngfuncs.GetTrackerIDForPlayer(playerIndex);
		if (userID > 0)
			return userID;
		return 0;
	}
	...
	{
		int playerSlot = gEngfuncs.GetPlayerForTrackerID(trackerID);
		if (!playerSlot)
			return NULL;

		hud_player_info_t playerInfo;
		gEngfuncs.pfnGetPlayerInfo(playerSlot, &playerInfo);
