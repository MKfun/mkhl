...
void COptionsSubKeyboard::ParseActionDescriptions( void )
{
    ...
    CUtlBuffer buf( (int)0, size + 1, true );
    // HACKHACK: fix ambigious constructor with int cast
...
void COptionsSubKeyboard::FillInDefaultBindings( void )
{
    ...
    int size = g_pFullFileSystem->Size(fh);
    // DLLHACK: cast to int to fix ambigious constructor
    CUtlBuffer buf( (int)0, size, true );
    g_pFullFileSystem->Read( buf.Base(), size, fh );
    g_pFullFileSystem->Close(fh);
...
// Purpose: Message map
//-----------------------------------------------------------------------------
vgui2::MessageMapItem_t COptionsSubKeyboard::m_MessageMap[] =
    {
