...
COptionsSubVideo::COptionsSubVideo(vgui2::Panel *parent) : PropertyPage(parent, NULL)
{
    ...
    unicodeText = g_pVGuiLocalize->Find("#GameUI_Software");
    g_pVGuiLocalize->ConvertUnicodeToANSI(unicodeText, m_pszRenderNames[0], 32);
    unicodeText = g_pVGuiLocalize->Find("#GameUI_OpenGL");
    g_pVGuiLocalize->ConvertUnicodeToANSI(unicodeText, m_pszRenderNames[1], 32);
    unicodeText = g_pVGuiLocalize->Find("#GameUI_D3D");
    g_pVGuiLocalize->ConvertUnicodeToANSI(unicodeText, m_pszRenderNames[2], 32);
...
void COptionsSubVideo::GetVidSettings()
{
    ...
    int f = 0;
    int d = 0;
    gameuifuncs->GetCurrentRenderer( p->renderer, 128, &p->windowed, &f, &t, &d);
    strlwr( p->renderer );
