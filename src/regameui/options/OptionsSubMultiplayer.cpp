...
COptionsSubMultiplayer::COptionsSubMultiplayer(vgui2::Panel *parent)
    : vgui2::PropertyPage(parent, "OptionsSubMultiplayer")
{
...
{
	// cleanup old remap files
	g_pFullFileSystem->RemoveFile("models/player/remapped.bmp", NULL);
	...
	sprintf(directory, "models/player/*.*");
...
// Purpose:
//-----------------------------------------------------------------------------
static void PaletteHueReplace(BMP_RGBQUAD *palSrc, int newHue, int Start, int end)
{
	int i;
	float r, b, g;
	float maxcol, mincol;
...
void COptionsSubMultiplayer::RemapPalette(char *filename, int topcolor, int bottomcolor)
{
	...
	if (bmfHeader.bfType == DIB_HEADER_MARKER)
	{
		...
		//char *pDIB = (char*)malloc(dwBitsSize); //(LPSTR)GlobalLock((HGLOBAL)hDIB);
		CUtlMemory<byte> dibData(0, dwBitsSize);
		byte *pDIB = dibData.Base();

		{
			g_pFullFileSystem->Read(pDIB, dwBitsSize, file);
			uint32 headerSize = *reinterpret_cast<uint32 *>(pDIB);

			//lpbmi = (BMP_RGBTRIPLE)pDIB;
			BMP_RGBQUAD *pPalette = reinterpret_cast<BMP_RGBQUAD *>(pDIB + headerSize);
			// BMP_RGBQUAD* color = reinterpret_cast<BMP_RGBQUAD*>(pColorTable) + i;
			// Remap palette
			PaletteHueReplace(pPalette, topcolor, SUIT_HUE_START, SUIT_HUE_END);
			PaletteHueReplace(pPalette, bottomcolor, PLATE_HUE_START, PLATE_HUE_END);
...
void COptionsSubMultiplayer::RemapLogoPalette(char *filename, int r, int g, int b)
{
	...
	if (bmfHeader.bfType == DIB_HEADER_MARKER)
	{
...

		{
			...
			// bool bWinStyleDIB = (headerSize >= 40);
			/* is this a Win 3.0 DIB? */
			bool bWinStyleDIB = true; // IS_WIN30_DIB(lpbi);

			float f = 0;
			byte *pColorTable = pDIB + headerSize;
			...
			for (int i = 0; i < 256; i++)
			{
				...
				{
					BMP_RGBQUAD *color = reinterpret_cast<BMP_RGBQUAD *>(pColorTable) + i;
					color->rgbRed = (byte)(r * t);
					color->rgbGreen = (byte)(g * t);
					color->rgbBlue = (byte)(b * t);
				}
				else
				{
					BMP_RGBTRIPLE *color = reinterpret_cast<BMP_RGBTRIPLE *>(pColorTable) + i;
					color->rgbtRed = (byte)(r * t);
					color->rgbtGreen = (byte)(g * t);
					color->rgbtBlue = (byte)(b * t);
				}
				f += 1.0f;
			}
...
void COptionsSubMultiplayer::OnApplyChanges()
{
	...
	if (bmfHeader.bfType == DIB_HEADER_MARKER)
	{
		...
		// UpdateLogoWAD((void *)dibData.Base(), r, g, b);
