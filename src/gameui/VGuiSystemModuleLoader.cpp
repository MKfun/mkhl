...
void CVGuiSystemModuleLoader::LoadPlatformModules(CreateInterfaceFn *factorylist, int factorycount, bool useSteamModules)
{
	...
	if (!kv->LoadFromFile(vgui2::filesystem(), "resource/PlatformMenu.vdf", "PLATFORM"))
		return;
	...
	for (KeyValues *it = menuKeys->GetFirstSubKey(); it != NULL; it = it->GetNextKey())
	{
		...
		vgui2::filesystem()->GetLocalCopy(dllPath);

		// load the dll
		char szDir[512];
		//MODDD - GetLocalPath LAZY DEFAULT   oh wait, not so lazy now
		if (!vgui2::filesystem()->GetLocalPath(dllPath, szDir, 512))
		{
...
/*
	...
	const char *filename = vgui2::filesystem()->FindFirst("AddOns/*.", &findHandle);

    for ( ; filename != NULL ; filename = vgui2::filesystem()->FindNext(findHandle))
//    while (filename)
    {
        // skip the . directories
        if (vgui2::filesystem()->FindIsDirectory ( findHandle ) && filename[0] != '.')
        {
            // add this to list
            ...
            if (!kv->LoadFromFile(vgui2::filesystem(), fileName, true, "PLATFORM"))
                continue;

            sprintf(fileName, "AddOns/%s/%s.dll", gameName, gameName);
            vgui2::filesystem()->GetLocalCopy(fileName);
...
*/
	...
	for (KeyValues *it = menuKeys->GetFirstSubKey(); it != NULL; it = it->GetNextKey())
	{
		...
		CSysModule* mod = vgui2::filesystem()->LoadModule(dllPath);//, "EXECUTABLE_PATH");
		if (!mod)
		{
...
void CVGuiSystemModuleLoader::ShutdownPlatformModules()
{
	...
	for (i = 0; i < m_Modules.Size(); i++)
	{
		vgui2::ivgui()->PostMessage(m_Modules[i].moduleInterface->GetPanel(), new KeyValues("Command", "command", "Quit"), NULL, 0);
	}
...
bool CVGuiSystemModuleLoader::ActivateModule(int moduleIndex)
{
	...
	if (g_pTaskbar)
	{
		...
		wTitle = vgui2::localize()->Find(m_Modules[moduleIndex].data->GetName());

		if(!wTitle)
		{
			vgui2::localize()->ConvertANSIToUnicode(m_Modules[moduleIndex].data->GetName(),w_szTitle,sizeof(w_szTitle));
			wTitle = w_szTitle;
		}
...
void CVGuiSystemModuleLoader::PostMessageToAllModules(KeyValues *message)
{
	for (int i = 0; i < m_Modules.Size(); i++)
	{
		vgui2::ivgui()->PostMessage(m_Modules[i].moduleInterface->GetPanel(), message->MakeCopy(), NULL, 0);
	}
	message->deleteThis();
}
