...
void CCreateMultiplayerGameGameplayPage::OnApplyChanges()
{
	...
	if ( m_pDescription )
	{
		...
		fp = IFileSystem()->Open( "settings.scr", "wb", NULL );
		if ( fp )
		{
			m_pDescription->WriteToScriptFile( fp );
			IFileSystem()->Close( fp );
		}
	}
}
...
void CServerDescription::WriteScriptHeader( FileHandle_t fp )
{
	...
	IFileSystem()->FPrintf( fp, (char *)getHint() );

// Write out the comment and Cvar Info:
	IFileSystem()->FPrintf( fp, "// Half-Life Server Configuration Layout Script (stores last settings chosen, too)\r\n" );
	IFileSystem()->FPrintf( fp, "// File generated:  %.19s %s\r\n", asctime( &newtime ), am_pm );
	IFileSystem()->FPrintf( fp, "//\r\n//\r\n// Cvar\t-\tSetting\r\n\r\n" );

	IFileSystem()->FPrintf( fp, "VERSION %.1f\r\n\r\n", SCRIPT_VERSION );

	IFileSystem()->FPrintf( fp, "DESCRIPTION SERVER_OPTIONS\r\n{\r\n" );
}
...
void CServerDescription::WriteFileHeader( FileHandle_t fp )
{
	...
	IFileSystem()->FPrintf( fp, "// Half-Life Server Configuration Settings\r\n" );
	IFileSystem()->FPrintf( fp, "// DO NOT EDIT, GENERATED BY HALF-LIFE\r\n" );
	IFileSystem()->FPrintf( fp, "// File generated:  %.19s %s\r\n", asctime( &newtime ), am_pm );
	IFileSystem()->FPrintf( fp, "//\r\n//\r\n// Cvar\t-\tSetting\r\n\r\n" );
}
