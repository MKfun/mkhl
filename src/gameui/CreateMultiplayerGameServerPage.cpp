...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CCreateMultiplayerGameServerPage::CCreateMultiplayerGameServerPage(vgui2::Panel *parent, const char *name) : PropertyPage(parent, name)
{
...
void CCreateMultiplayerGameServerPage::LoadMapList()
{
	...
	const char *filename = IFileSystem()->FindFirst("maps/*.bsp", &findHandle, NULL);
	while (filename)
	{
		...
		// get the next file
	nextFile:
		filename = IFileSystem()->FindNext(findHandle);
	}
	IFileSystem()->FindClose(findHandle);
