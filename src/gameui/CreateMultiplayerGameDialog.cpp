...
void CCreateMultiplayerGameDialog::OnOK()
{
	...
	if (!stricmp( ModInfo().GetGameDescription(), "Counter-Strike" ))
	{
		...
		vgui2::RadioButton *noBots = static_cast<vgui2::RadioButton *>( m_pServerPage->FindChildByName( "NoBots" ) );
		if (noBots && !noBots->IsSelected())
		{
			// "not no bots" -> use bots

			vgui2::RadioButton *easy = static_cast<vgui2::RadioButton *>( m_pServerPage->FindChildByName( "SkillLevelEasy" ) );
			if (easy && easy->IsSelected())
				botSkill = 0.0f;

			vgui2::RadioButton *normal = static_cast<vgui2::RadioButton *>( m_pServerPage->FindChildByName( "SkillLevelNormal" ) );
			if (normal && normal->IsSelected())
				botSkill = 1.0f;

			vgui2::RadioButton *hard = static_cast<vgui2::RadioButton *>( m_pServerPage->FindChildByName( "SkillLevelHard" ) );
			if (hard && hard->IsSelected())
				botSkill = 2.0f;

			vgui2::RadioButton *expert = static_cast<vgui2::RadioButton *>( m_pServerPage->FindChildByName( "SkillLevelExpert" ) );
		...
		m_pBotSavedData->SaveToFile( IFileSystem(), "CSBotConfig.vdf", "CONFIG" );
