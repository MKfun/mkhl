...
DWORD WINAPI myRenderThread(void* arg_data) {
	...
	// && !stopDaLoop
	//while (vgui2::ivgui()->IsRunning()){
	while(!stopDaLoop){

		vgui2::VPANEL funnyMusic = vgui2::surface()->GetEmbeddedPanel();
...
/*	CreateInterfaceFn fileSystemFactory = factories[ 2 ];
	...
	vgui2::VGui_InitInterfacesList("GameUI", m_FactoryList, count); // client factory may not be passed in
*/
	...
	*/
	...
	//vgui2::VGui_InitInterfacesList( "GameUI", m_FactoryList, 2 );
	vgui2::VGui_InitInterfacesList( "GameUI", m_FactoryList, finalCount );
	...
	*/
	...
	vgui2::StringIndex_t testiii = vgui2::localize()->FindIndex("doesnotexist");

	BOOL daMatch = (testiii == vgui2::INVALID_STRING_INDEX);
	...
	*/
	...
	enginesurfacefuncs = (vgui2::ISurface * )engineFactory(VGUI_SURFACE_INTERFACE_VERSION,NULL);
...
void CGameUI::MasterVersionCheckQuery()
{
	...
	if (kv->LoadFromFile(vgui2::filesystem(), "Servers/MasterServers.vdf", true))
	{
...
void CGameUI::GetUpdateVersion( char *pszProd, char *pszVer)
{
	...
	if ( vgui2::filesystem()->FileExists("valve.inf") )
	{
		vgui2::filesystem()->GetLocalPath("valve.inf", szFileName, MAX_PATH); // GetPrivateProfileSection() is an OS call...needs the whole file at hand.
	...
	if (FindPlatformDirectory(m_szPlatformDir, sizeof(m_szPlatformDir)))
	{
		...
		if (vgui2::system()->GetRegistryString("HKEY_LOCAL_MACHINE\\Software\\Valve\\Steam\\Language", language, sizeof(language)))
		{
			if (strlen(language) > 0 && stricmp(language, "english"))
			{
				...
				vgui2::filesystem()->AddSearchPath(path, "PLATFORM");
			}
		}
...
// FOR SRC
//	vgui2::surface()->SetWorkspaceInsets( 0, 0, 0, g_pTaskbar->GetTall() );
...
bool CGameUI::FindPlatformDirectory(char *platformDir, int bufferSize)
{
	...
	if (platformDir[0] == '\0')
	{
		...
		if (vgui2::filesystem()->FileExists(pszServerBrowserDLL))
		{
			char szPlatformPath[MAX_PATH];
			char szFinalPath[MAX_PATH];
			//MODDD - GetLocalPath LAZY DEFAULT. not so lazy now.
			vgui2::filesystem()->GetLocalPath(pszServerBrowserDLL, szPlatformPath, MAX_PATH);
...
/*	// this function exported by cdkey.obj (we don't have the source code)
	...
	char cdkey[255];
	if (!force && (!engine->CheckParm("-forcevalve", NULL) && vgui2::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Half-Life\\Settings\\Key", cdkey, sizeof(cdkey))
			&& strlen(cdkey) > 0) )
	{
		if ( SimpleCDCheck(cdkey) == 1 )
		{
			// success
			// vgui2::ivgui()->DPrintf2("CD KEY %s VALIDATED\n", cdkey);
			return;
		}
	}
	else if (!force && vgui2::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Half-Life\\Settings\\ValveKey", cdkey, sizeof(cdkey))
			&& strlen(cdkey) > 0)
	{
	...
	else if (!force && vgui2::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Half-Life\\Settings\\yeK1", cdkey, sizeof(cdkey)))
	{
		// TODO: Add weak encrypted key validation check here!
		if (strlen(cdkey) == 254)
		{
			if(vgui2::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\Valve\\Half-Life\\Settings\\yeK2", cdkey, sizeof(cdkey)))
			{
				if(strlen(cdkey) == 108)
				{
					// success
	...
	vgui2::ivgui()->DPrintf2("CD KEY INVALID\n");
