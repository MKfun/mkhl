...
#include "vgui/spectator_panel.h"
#include "hltv.h"
...
void SpectatorMode(void)
{
	...
	if (gEngfuncs.Cmd_Argc() == 2)
		CHudSpectator::Get()->SetModes(atoi(gEngfuncs.Cmd_Argv(1)), -1);
	else if (gEngfuncs.Cmd_Argc() == 3)
		CHudSpectator::Get()->SetModes(atoi(gEngfuncs.Cmd_Argv(1)), atoi(gEngfuncs.Cmd_Argv(2)));
}
...
DECLARE_HUDELEMENT(CHudSpectator);
...
void CHudSpectator::SetSpectatorStartPosition()
{
	...
	VectorCopy(m_cameraOrigin, pm_vJumpOrigin);
	VectorCopy(m_cameraAngles, pm_vJumpAngles);
...
void CHudSpectator::SetCameraView(const Vector &pos, const Vector &angle, float fov)
{
	...
	VectorCopy(angle, pm_vJumpAngles);
	gEngfuncs.SetViewAngles(pm_vJumpAngles);
	pm_iJumpSpectator = 1; // jump anyway
}
...
void CHudSpectator::DirectorMessage(int iSize, void *pbuf)
{
	...
	case DRC_CMD_CAMERA:
		...
		v1[1] = READ_COORD();
		v1[2] = READ_COORD(); // pm_vJumpOrigin

		v2[0] = READ_COORD(); // view angle
		v2[1] = READ_COORD(); // pm_vJumpAngles
		v2[2] = READ_COORD();
		f1 = READ_BYTE(); // fov
		i1 = READ_WORD(); // target
	...
	case DRC_CMD_MESSAGE:
	{
		...
		CHudMessage::Get()->MessageAdd(msg);
	...
	case DRC_CMD_BANNER:
		...
		//g_pViewport->GetSpectator()->m_TopBanner->LoadImage(READ_STRING());
		//g_pViewport->UpdateSpectatorPanel();
		break;
...
void CHudSpectator::FindNextPlayer(bool bReverse)
{
	...
	else
	{
		...
		VectorCopy(pEnt->origin, pm_vJumpOrigin);
		VectorCopy(pEnt->angles, pm_vJumpAngles);
	}
...
void CHudSpectator::FindPlayer(const char *name)
{
	...
	for (int i = 1; i <= MAX_PLAYERS; i++)
	{
		...
		if (!_stricmp(GetPlayerInfo(i)->GetName(), name))
		{
			g_iUser2 = i;
			break;
		}
...
void CHudSpectator::HandleButtonsUp(int ButtonPressed)
{
	...
	if (!g_pViewport->GetSpectator()->IsVisible())
		return; // dont do anything if not in spectator mode
...
void CHudSpectator::SetModes(int iNewMainMode, int iNewInsetMode)
{
	...
	if (iNewMainMode != g_iUser1)
	{
		...
		case OBS_ROAMING: // jump to current pm_vJumpOrigin/angle
			g_iUser1 = OBS_ROAMING;
			if (g_iUser2)
			{
				V_GetChasePos(g_iUser2, v_cl_angles, pm_vJumpOrigin, pm_vJumpAngles);
				gEngfuncs.SetViewAngles(pm_vJumpAngles);
				pm_iJumpSpectator = 1;
			}
			break;
...
{
	return (ent && ent->player && ent->curstate.solid != SOLID_NOT && ent != gEngfuncs.GetLocalPlayer() && GetPlayerInfo(ent->index)->IsConnected());
}
